...
{
  /*
   * ★同じ名前が 2 本の木にあるとき、持ち主は**先に見つけたほう**
   *   （2026-09-05 本人の報告で入れ替えた）。
   *
   *   > 響きで「dinosur jr」をいろんなパターンで生成したところ、
   *   > 最初に生成してできたバンドが他のリストに分散してどんどん少なくなりました。
   *
   * 前は「重いほう」で、重みには savedAt が入っている（新しいほど強い）ため、
   * **辿り直すたびに古い木から名前が抜かれていた。**増やしたつもりが移動だった。
   * ★1 本ずつ完結させる設計（main.js の「すでにある は渡さない」）とも食い違っていた。
   */
  const 生 = {
    version: 1,
    entries: [
  ...
  確認(
    '★同じ名前が 2 本にあれば、先に見つけたほうが持ち主',
    m.当たり.length === 1 && m.当たり[0].keyword === '古いほう',
    JSON.stringify(m.当たり.map((a) => a.keyword)) + '（あとの木が奪うと、古い一覧が痩せます）',
  );
  確認(
    '★理由も、先に見つけたほうのもの',
    m.当たり.length === 1 && m.当たり[0].description === '古い理由',
    JSON.stringify(m.当たり.map((a) => a.description)),
  );
  /*
   * ★辿り直しても、先に見つけた木は痩せない（本人の報告そのもの）。
   * 同じ言葉を 3 回 辿って、1 本目が持っていた名前が減らないことを見る。
   */
  {
    const 三回 = {
      version: 1,
      entries: [
        { keyword: 'dinosaur jr', savedAt: '2026-09-01T00:00:00.000Z',
          nodes: [{ name: 'Converge', genre: 'music', description: 'a', depth: 1 },
            { name: 'Someone', genre: 'music', description: 'b', depth: 1 }] },
        { keyword: 'dinosaur jr(01)', savedAt: '2026-09-02T00:00:00.000Z',
          nodes: [{ name: 'Converge', genre: 'music', description: 'c', depth: 1 }] },
        { keyword: 'dinosaur jr(02)', savedAt: '2026-09-03T00:00:00.000Z',
          nodes: [{ name: 'Converge', genre: 'music', description: 'd', depth: 1 },
            { name: 'Someone', genre: 'music', description: 'e', depth: 1 }] },
      ],
    };
    const r = 突き合わせる(読み込む(JSON.stringify(三回)).木, 曲たち, いま);
    const 一本目 = r.当たり.filter((a) => a.keyword === 'dinosaur jr').length;
    確認(
      '★同じ言葉を辿り直しても、1 本目は痩せない',
      一本目 === 2,
      `1 本目 ${一本目} 組（2 のはず。減ると「増やしたのに減った」になります）`,
    );
    /*
     * ★どちらの木で絞っても出る（2026-09-05 本人の報告）。
     *
     *   > dinosaur jr から響きで選んだバンドで
     *   > 王道のプレイリストと外しのプレイリストを作りたかった
     *   > 結果は外しが wrecking crew だけになってしまった
     *
     * 持ち主を 1 本に決めるだけだと、**どちらかが必ず痩せる。**
     *   新しいほうが奪う … 1 本目が痩せる
    ...
    確認(
      '★曲は、挙げた木すべての言葉を持つ',
      !!(曲 && Array.isArray(曲.言葉たち) && 曲.言葉たち.length === 3),
      JSON.stringify(曲 && 曲.言葉たち) + '（1 本しか持てないと、どちらかの一覧が痩せます）',
    );
    確認(
      '★持ち主（理由に使う言葉）は、いままでどおり 1 つ',
      !!(曲 && 曲.keyword === 'dinosaur jr'),
      String(曲 && 曲.keyword),
    );
  }
}
 ...
 */
{
  ...
  確認(
    '★辿るたびに貯める',
    素画面.includes('await 無い名前を貯める(辿った名前 || v);')
   ...
   */
  /*
   * ★一言 解説つきで書き写す（2026-09-04 本人の希望で変えた）。
   *   > バンド名だけは寂しかった。リストの横の一言解説はかなりありがたく
   *   > 重要なのでこれもコピペしたい。
   * 最初は名前だけにしていた（検索窓へそのまま貼れるように）。だが貼った先で
   * 「どれが何だか分からない」。解説こそが、探しに行くかを決める材料だった。
   */
  確認(
    '★一言 解説とジャンルの見出しごと書き写せる',
    素画面.includes('【' + '$' + '{見出し}】')
      && 素画面.includes('x.description ? `' + '$' + '{x.name} ― ' + '$' + '{x.description}`')
      && 素本体.includes("ipcMain.handle('clip:write'"),
    '名前だけだと、貼った先でどれが何だか分かりません',
  );
  /*
   * ★辿った直後は、いま辿った組を開く（2026-09-04 本人の希望）。
   *   > バンドから辿るを実行した場合、その時に生成したものが開いてくれると嬉しい
   */
  確認(
    '★辿った直後は、その組を開く',
    素画面.includes('if (辿った言葉) 無い名前の開いた組 = 起点のジャンル(辿った言葉) || 辿った言葉 ||'),
    'いちばん多い組が開くと、いま出たものが埋もれます',
  );
  /*
   * ★「辿った言葉」の列は、1 曲が複数の値を持つ（2026-09-05 本人の報告）。
   * 同じ名前を 2 本の木が挙げたら、どちらで絞っても出る。
   * 絞り込みも列の中身も、ほどいてから扱わないと片方が消える。
   */
  確認(
    '★辿った言葉の列は、曲の持つ言葉すべてを見る',
    素画面.includes("{ key: '言葉', 見出し: '辿った言葉', 取る: (t) => 響きの印(t, '言葉たち') }")
      && /function 値たち\(v\)/.test(素画面),
    '1 つに決めると、王道の池か外しの池のどちらかが痩せます',
  );
  確認(
    '★絞り込みも列の中身も、ほどいてから扱う',
    素画面.includes('const vs = 値たち(列[i].取る(t));')
      && 素画面.includes('const vs = 値たち(取る(t));')
      && 素画面.includes('const 素材 = 絞る(level).flatMap((t) => 値たち(値を取る(t)));'),
    'ほどかないと、配列がそのまま名前として並びます',
  );
  確認(
    '★(01) が付いた言葉でも、起点のジャンルを引ける',
    素画面.includes(String.raw`.replace(/\(\d+\)$/, '')`),
    '2 度目に辿ると helmet(01) になり、手元の演者に当たらなくなります',
  );
  /*
