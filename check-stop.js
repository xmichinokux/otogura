...
(async () => {
  ...
  console.log('\n[8] ★走査している間も、画面が動くか（息継ぎ）');
  /*
   * ■ 実地の報告（2026-08-29、3 回目）:
   *   > また文字が入力できないバグが発生しました。
   *   > 時間が経つと文字が打てるようになりました。裏の処理の関係ですか？
   *   > 理由がわからないと壊れたと感じます。
   *
   * ★理由は「走査が本体を占領していた」だった。
   * Electron の主プロセスは Chromium の browser プロセスでもあるので、
   * ここが詰まると**キー入力が画面に届かない。**
   * 走査が進むと空くので、待てば打てるようになる ―― 報告のとおりだった。
   *
   * ★2 回、再現できずに終わっている。**速さではなく「詰まり」を測る**と出た。
   * だから、ここで測るのは走査の速さではなく**詰まりの長さ**。
   */
  {
    const { 根 } = 台を作る(600);

    // 100 ms ごとに起きるはずのものが、どれだけ遅れるか
    const 遅れ = [];
    let 前 = Date.now();
    const 時計 = setInterval(() => { const 今 = Date.now(); 遅れ.push(今 - 前 - 100); 前 = 今; }, 100);
    await scanLibrary([根], [], {}, null, null, null, () => false);
    clearInterval(時計);
    fs.rmSync(根, { recursive: true, force: true });

    const 最悪 = 遅れ.length ? Math.max(...遅れ) : 0;
    確認(
      '★走査中に、長く詰まらない',
      最悪 < 200,
      `いちばん長い詰まり ${最悪} ms（200 ms を超えると、押した感じが無くなります）`,
    );

    /*
     * ★息継ぎが入っているか、書いてあることも見る。
     * 上の測りは機械の速さに左右される（速い機械だと、詰まっていても通る）。
     * 測りと突き合わせの両方で守る。
     */
    const 本文 = fs.readFileSync(path.join(__dirname, 'src/library.js'), 'utf8');
    const 素 = 本文.replace(/\/\*[\s\S]*?\*\//g, ' ').replace(/^\s*\/\/.*$/gm, ' ');
    確認(
      '★1 曲読むごとに息継ぎしている',
      /await 一本\(対象\[i\]\);[\s\S]{0,80}?await 息継ぎ\(\)/.test(素),
      '詰めると、走査中ずっと打てなくなります',
    );
    確認(
      '★数えている間も息継ぎしている',
      /await 息継ぎ\(\);[\s\S]{0,60}?for \(const e of entries\)/.test(素),
      '数えるだけで 297 秒かかるので、ここも詰まります',
    );
  }
