 ...
 */
const 幅の段 = [
  { 札: '狭い',     候補: 50,  ジャンル: 2,  名前: 6,  枝: ['2 個', '1 個ずつ', '1 個ずつ'] },
  { 札: 'やや狭い', 候補: 100, ジャンル: 3,  名前: 12, 枝: ['3 個', '1〜2 個ずつ', '1 個ずつ'] },
  { 札: 'ふつう',   候補: 200, ジャンル: 5,  名前: 21, 枝: ['3〜4 個', '2 個ずつ', '1〜2 個ずつ'] },
  { 札: 'やや広い', 候補: 400, ジャンル: 8,  名前: 35, 枝: ['5 個', '2 個ずつ', '2 個ずつ'] },
  { 札: '広い',     候補: 800, ジャンル: 12, 名前: 60, 枝: ['6 個', '3 個ずつ', '2 個ずつ'] },
];
...
async function プレイリストを作らせる({ キー, 気分, 候補, 曲数 = 作る曲数, 響き = '' }) {
  ...
  try {
    ...
    const 返り = await client.messages.parse({
      ...
      system: プレイリストの頼み文(気分.trim(), 頼む曲数, 響き),
    ...
    return { ok: true, 結果: { ...並びを確かめる(出, 候補), 頼んだ曲数: 頼む曲数 } };
  } catch (e) {
    const 名 = e && e.constructor ? e.constructor.name : '';
    if (名 === 'AuthenticationError') return { ok: false, error: 'APIキーが正しくないようです' };
    if (名 === 'RateLimitError') return { ok: false, error: '短い間に呼びすぎました。少し待ってからもう一度' };
...
function 木の頼み文(手元の演者, 幅 = 幅の段[既定の目盛 - 1]) {
 ...
 *        | { ok: false, error }
 */
async function 木を生やす({ キー, 言葉, 手元の演者 = [], 幅目盛 }) {
  if (!キー) return { ok: false, error: 'APIキーが設定されていません' };
...
module.exports = { 使うモデル, 候補の数, 作る曲数, 見せる演者の数, 目盛の数, 既定の目盛, 幅の段, 量の段, 幅を読む, 量を読む, 曲数を丸める, ジャンル一覧を作る, 頼み文, 照らし合わせる, おすすめを聞く, プレイリストの頼み文, 並びを確かめる, プレイリストを作らせる, 木の頼み文, 木を生やす };
