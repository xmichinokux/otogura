...
const 強度の段 = [
  {
    ...
    一本: [
      ...
      '・★候補の（）の数は、**この範囲でその演者を何曲持っているか**です。',
      '　数の大きい演者を軸にしてください。**その人にとっての王道**です',
    ],
  },
  {
    札: 'やや王道', effort: 'low',
    ...
    一本: [
      ...
      '・候補の（）の数は、この範囲でその演者を何曲持っているかです。大きいものを軸に',
    ],
  },
  {
    札: 'ふつう', effort: 'medium',
    ...
    一本: [
      ...
      '・候補の（）の数は、この範囲でその演者を何曲持っているかです。大小を混ぜて',
    ],
  },
  {
  ...
  },
  {
    ...
    一本: [
      ...
      '・★候補の（）の数は、**この範囲でその演者を何曲持っているか**です。',
      '　**1〜2 曲しか持っていない演者を積極的に**選んでください。',
      '　たくさん持っている演者は、もう聴いています',
    ],
  },
];
/** 量の段（作る曲数） */
...
async function プレイリストを作らせる({ キー, 気分, 候補, 曲数 = 作る曲数, 響き = '', 強度目盛 }) {
  ...
  for (const c of 候補) {
    ...
    if (k) 演者ごとの数.set(k, (演者ごとの数.get(k) ?? 0) + (c.曲数 ?? 1));
  }
  const 表 = 候補
    .map((c) => {
      const k = String(c.artist || '').toLocaleLowerCase('ja').trim();
      const n = c.曲数 ?? 演者ごとの数.get(k) ?? 1;
      return `${c.番号}\t${短く(c.artist, 28)}(${n})\t${短く(c.title, 40)}\t${短く(c.album, 28)}`;
    })
    .join('\n');
  const かたち = z.object({
    題: z.string(),
  ...
  try {
    ...
    const 返り = await client.messages.stream({
      ...
      messages: [{ role: 'user', content: '■ 候補（番号／アーティスト(この範囲で持っている曲数)／曲名／アルバム）\n' + 表 }],
      output_config: { effort: 'low', format: zodOutputFormat(かたち) },
    }).finalMessage();
