...
function 頼み文(ジャンル一覧, 年一覧) {
  ...
  return [
    'あなたは、その人が書いた短い言葉を読んで、まず 2 つに振り分ける役です。',
    '',
    '  気分 … そのときの状態や望みを言った言葉',
    '  文脈 … アーティスト名・作品名・場所・思想など、何かの名前',
    '',
    '★迷ったら「文脈」にしてください。名前なら、そこから辿って探す道があります。',
    '★「気分」と読んだときだけ、下の一覧から絞り込みを選んでください。',
    '　「文脈」と読んだときは、ジャンルも年も空でかまいません（別の道で扱います）。',
    '',
    'どちらの場合も、曲は選びません。**絞り込みの条件だけ**です。',
    '',
...
function 照らし合わせる(返事, 実在ジャンル, 実在年) {
  ...
  return {
    /*
     * ★AI がどちらと読んだか。画面に出して、読み違いが見えるようにする。
     * 黙って振り分けると、なぜその選曲になったのか分からなくなる。
     * 分からない値が返ったら「気分」に倒す（そちらが今までの動き）。
     */
    種類: (返事 && 返事.種類 === '文脈') ? '文脈' : '気分',
    ジャンル: g.通った,
    年: y.通った,
