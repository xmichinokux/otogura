...
const 強度の段 = [
  ...
  },
  {
    ...
    流れ: [
      '・★後半は、**離れたところまで手を伸ばしてよい**です',
      '　前半で足場を作り、後半で少しずつ広げていく',
    ],
    一本: [
...
function プレイリストの頼み文(気分, 曲数, 響き = '', 演者の数 = 0, 強度 = 強度の段[既定の目盛 - 1]) {
  return [
 ...
 */
async function プレイリストを作らせる({ キー, 気分, 候補, 曲数 = 作る曲数, 響き = '', 強度目盛 }) {
  if (!キー) return { ok: false, error: 'APIキーが設定されていません' };
  if (typeof 気分 !== 'string' || !気分.trim()) return { ok: false, error: '気分が空です' };
