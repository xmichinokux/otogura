  ...
  for (const e of 木.木) {
    for (const n of e.nodes) {
      ...
      if (!前) {
        ...
        /*
         * ★持ち主は「**先に見つけた木**」にする（2026-09-05 本人の報告）。
         *
         *   > 響きで「dinosur jr」をいろんなパターンで生成したところ、
         *   > 最初に生成してできたバンドが他のリストに分散してどんどん少なくなりました。
         *   > 選ばれる数は決まっていて、それがリストが増えるごとに分散する感じです。
         *
         * ★前は「重いほう」だった。重みには savedAt が入っている（新しいほど強い）ので、
         * **辿り直すたびに、古い木から名前が抜かれていた。**
         * dinosaur jr が 4 組、dinosaur jr(02) が 7 組 ―― 増やしたつもりが、
         * 移動していただけ。全体（125 組）は変わっていない。
         *
         * ★設計とも食い違っていた。main.js にこう書いてある:
         *   > 「すでに挙げた名前」は渡さない。生成ごとに別物として残すので、
         *   > 1 本ずつ完結していてよい。
         * 1 本ずつ完結させる気なら、**あとから来た木が奪ってはいけない。**
         *
         * ★先着にすると、それぞれの木の意味がはっきりする。
         *   1 本目 … 最初に見つけたぶん（**減らない**）
         *   2 本目 … そのうえで**新しく増えたぶん**
         * 「辿るほど増える」が、そのまま画面に出る。
         *
         * ★日付が同じ・無いときだけ、これまでどおり重いほうを採る。
         */
        const 前の日 = Date.parse(前.savedAt || '') || Infinity;
        const 今の日 = Date.parse(e.savedAt || '') || Infinity;
        const 先に見つけた = 今の日 < 前の日
          || (今の日 === 前の日 && 持ち主の重み > 前.持ち主の重み);
        if (先に見つけた) {
