...
const { おすすめを聞く, プレイリストを作らせる, 木を生やす, 候補の数, 作る曲数, 一組の曲数, 見せる演者の数,
...
function 目盛を整える(v) {
  ...
  /*
   * ★拡大解釈は目盛ではなく入／切（2026-08-30 本人の希望）。
   *   > 拡大解釈機能ですが、スライダーから外して独立したオンオフボタンにしたい
   *   > 王道の拡大解釈が意外と面白いと思った
   *
   * ★文脈の強度とは別の軸なので、目盛に混ぜず真偽で持つ。
   * 既定は切（0.27.0 までの王道側の動きに合わせる）。
   */
  return {
    幅: 目盛ひとつ(o.幅), 量: 目盛ひとつ(o.量), 強度: 目盛ひとつ(o.強度),
    拡大解釈: o.拡大解釈 === true,
  };
}
...
ipcMain.handle('ai:suggest', async (_e, 手がかり) => {
  ...
  return おすすめを聞く({
    ...
    年一覧: (手がかり && 手がかり.年一覧) || [],
    /*
     * ★演者一覧は**突き合わせにだけ使う。外へは送らない。**
     * AI が自分の知識で挙げた名前を、こちらで手元と照らすためのもの。
     * 送るものは今までどおり（ジャンル名・年・気分の文・候補 200 曲）。
     */
    演者一覧: (手がかり && 手がかり.演者一覧) || [],
    /*
     * ★つまみ（王道↔外す・拡大解釈）を、この段にも渡す（2026-08-31 本人の報告）。
     *   > 拡大解釈としてはかなり好きなセレクトなんですが、
     *   > 僕としてはロックの中から選んでほしかったなという感じです。
     * 王道に振ってあるのに広がっていた。強度が届いていなかった。
     */
    強度目盛: 目盛.強度,
    拡大解釈: 目盛.拡大解釈 === true,
    幅目盛: 目盛.幅,
...
ipcMain.handle('ai:sizes', async () => {
  ...
  return {
    候補の数: 幅を読む(目盛.幅).候補,
    作る曲数: 量を読む(目盛.量).曲数,
    /* ★1 組（演者）から何曲、候補に入れるか。画面はこれを見て候補を作る */
    一組の曲数: 一組の曲数(幅を読む(目盛.幅).候補, 量を読む(目盛.量).曲数),
    目盛,
    目盛の数,
    幅の段,
    量の段,
...
ipcMain.handle('ai:playlist', async (_e, 手がかり) => {
  ...
  return プレイリストを作らせる({
    ...
    候補: 手がかり && 手がかり.候補,
    /*
     * ★画面から来るのは**上限だけ**。増やす方向には効かない（2026-08-31）。
     * つまみは今までどおりここで読む。画面の言い値で増やせるようにすると、
     * 置き場を 1 つにした意味が無くなる。
     * 演者で絞っているときに、組の数より多く頼まないためのもの。
     */
    曲数: Math.min(量を読む(目盛.量).曲数,
      (手がかり && Number(手がかり.組の数) > 0) ? Number(手がかり.組の数) : Infinity),
    強度目盛: 目盛.強度,
    拡大解釈: 目盛.拡大解釈 === true,
...
ipcMain.handle('ai:tree', async (_e, 手がかり) => {
  ...
  const r = await 木を生やす({
    ...
    /* ★数を埋めるかどうかは拡大解釈だけで決まる（2026-08-31 本人の整理） */
    拡大解釈: (await 設定を読む()).AIの目盛.拡大解釈 === true,
  });
  if (!r.ok) return r;
  ...
  /* ★混ぜるのはやめた（0.37.0）。前の同じものを探す必要も無くなった */
  /*
  ...
  return {
    ...
    /*
     * ★混ぜるのをやめたので、ここも 混ぜた ではなく生やしたぶん（2026-08-31）。
     * 消し忘れて ReferenceError になり、保存は済んでいるのに窓口が落ちて
     * 画面に返らない、という不具合になっていた。立ち上げ直すと出てきた。
     */
    全部で: r.結果.nodes.length,
    /* ★実際に保存した名前。helmet(01) のように変わることがある */
    名前: 名,
  };
});
...
ipcMain.handle('scan', async (e) => {
  ...
  /*
   * ★設定は**ここで読み直す**（2026-09-04 本人の報告で見つけた）。
   *
   * ■ 起きていたこと
   ...
   *
   * ★原因はここ。s は**走査を始める前**に読んだもの。93,856 曲では数分かかる。
   * その古い s を丸ごと書き戻していたので、**走査中にやった設定変更が
   * 走査の終わりに全部 巻き戻っていた。** 再生リストの削除も、AI が作った一本も、
   * フォルダの追加も、ジャンルのまとめも、つまみも。
   *
   * ★「そのあと 1 つ消したら直った」のは、そこで初めてディスクを読み直したから。
   * 壊れてはいなかった ―― 一度 巻き戻って、読み直しで揃っただけ。
   *
   * ★直し方は 1 つ。**書く直前に読み直して、触る欄だけ差し替える。**
   * 長く持った設定を丸ごと書き戻さない。
   */
  const 今の設定 = await 設定を読む();
  const 掃除 = 掃除する(今の設定.lists);
  if (掃除.落とした) { 今の設定.lists = 掃除.lists; await 設定を書く(今の設定); }
