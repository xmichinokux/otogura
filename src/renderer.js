...
function 組む範囲を書く() {
  ...
  /*
   * ★効いている選択だけを並べる（2026-08-31）。
   * 前は sel を丸ごと並べていたので、隠れたタブの選択まで書いていた。
   * 曲数には効いていないのに欄には出る ―― いちばん質の悪い食い違い。
   */
  const 効く鍵 = new Set((カラムタブの列[カラムタブ] || []).map((c) => c.key));
  const 絞り = Object.entries(sel)
    .filter(([k, v]) => v && 効く鍵.has(k))
