...
function まとめを入れる(v) {
  ...
  /*
   * ★まとめてあれば、既定はまとめのタブ（2026-08-31 本人の希望）。
   *   > 僕が体感的に使ってほしいのはAIにお願いしてジャンルをまとめたタブです。
   * 散らかった 98 個より、まとめた 18 組のほうが見て回りやすい。
   * ★自分でタブを触ったあとは変えない（選んだ操作を奪わない）。
   */
  if (!タブを触った && まとめてあるか()) カラムタブ = "まとめ";
}
...
/*
 * ★見えている 1 列目の選択だけが効く（2026-08-31 本人の指摘で作り直した）。
 *
 *   > 操作が煩雑で自分で色々試して仕様を調べないと正しく動かすことができない
 *   > 何かの仕様を捨てないとシンプルで直感的な操作ができない
 *
 * ★捨てた仕組みが 2 つある。どちらも**画面に出ていなかった。**
 *
 * ① 隠れているタブの選択も効く（2026-08-29 に本人の希望で入れた）。
 *    タブが 2 つ（ジャンル・日付）のころは、掛け合わせに意味があった。
 *    4 つになり、響きが暗黙に絞るようになってからは、混乱の源だった。
 *    ★掛け合わせ（ジャンル×日付）は使っていないと本人に確認済み。
 *
 * ② 響きタブにいるだけで、当たった曲だけに絞られる。
 *    sel に出ないので「絞りを外す」で消せなかった。**名前どおりに働いていなかった。**
 *    いまは「辿った言葉」の列で選んだときだけ絞る ―― ほかの列と同じ。
 *
 * ★これで、範囲は**画面を見れば分かる**ようになる。
 * 「誰が絞ったか覚える」ような仕掛けは要らない。見えないものが無いので。
 */
function 絞る(level) {
  const 列たち = カラムタブの列[カラムタブ] || [];
  return 見える曲().filter((t) => タブに合う(t, カラムタブ, level, 列たち));
}
...
/*
 * ★組む範囲は、画面を見ない（2026-09-02 本人の指定）。
 *
 *   > AI DJと響きは画面に表示されてるリストを参照しない。内部でリストを参照する
 *   > ジャンルを指定していなければ全てを参照するけど、ジャンルの指定をしていたら
 *   > そのジャンル内のみ参照する。しかし、画面に表示されているかされていないかは
 *   > 関係なく、あくまでジャンルを選択したとしても内部でリストを参照する
 *
 * ★0.40.0 で「隠れた絞り込み」は潰したが、**タブを開いているという状態そのものが
 * 範囲を決める**構造は残っていた。響きタブを見ているだけで、組む先が変わる。
 * それが「生成の誤操作」の最後の源だった。
 *
 * ★決まりは 2 行になる。
 *   ・組む範囲は、ジャンルの選択だけで決まる（まとめ優先、無ければ元の名前）
 *   ・アーティスト・アルバム・日付・響きの列は、**組む範囲に一切 効かない**
 *
 * ★再生とシャッフルは今までどおり（本人の指定は A）。
 * 「見えているもの＝鳴るもの」「ジャンルの選択＝組まれるもの」で規則が分かれる。
 */
/* 生成のときだけ使う内部の絞り込み。null なら「ジャンルの選択に従う」 */
let 組みの絞り = null;

/** 鍵から、その列の値の取り方を引く（タブをまたいで 1 か所にまとめる） */
function 取り方を引く(key) {
  for (const 列 of Object.values(カラムタブの列)) {
    for (const c of 列) if (c && c.key === key && c.取る) return c.取る;
  }
  return null;
}
...
/*
 * ジャンルの選択だけを取り出す。**まとめ優先、無ければ元の名前。**
 * ★日付は見ない（本人の指定）。散らかったジャンルより、まとめた組のほうが
 * AI の検索から漏れにくい ―― 実測で 98 個 対 18〜22 組。
 */
function 生成のジャンル絞り() {
  const 鍵 = まとめてあるか() ? 'まとめ' : 'genre';
  const 集 = sel[鍵];
  return (集 && 集.size) ? { [鍵]: 集 } : {};
}

function 組む範囲() {
  const 絞り = 組みの絞り || 生成のジャンル絞り();
  return 見える曲().filter((t) => 組みに合う(t, 絞り)).sort(曲を並べる);
}
...
function 組む範囲を書く() {
   ...
   */
  /*
   * ★出すのは「実際に組む範囲」（2026-09-02）。
   * 組む範囲は画面を見なくなったので、sel を並べると嘘になる。
   * アーティストで絞っていても、組む範囲には効かない ―― それをそのまま出す。
   * ★「見える」と「操作で変わる」を切り離す。見えなくしたわけではない。
   */
  const 効く絞り = 組みの絞り || 生成のジャンル絞り();
  ...
  /*
   * ★組む範囲に**実際に効いているもの**だけを並べる（2026-09-02）。
   * 前は「開いているタブの選択」を並べていた。組む範囲が画面を見なくなった
   * いまは、それでは食い違う。効く絞り をそのまま書く。
   */
  const 絞り = Object.entries(効く絞り)
    .filter(([, v]) => v && v.size)
 ...
 */
/*
 * ★「自分で絞ってある」は、**ジャンルを選んでいるか**だけを見る（2026-09-02）。
 * 前は sel のどれか 1 つでも選んでいれば真だった。演者やアルバムや日付で
 * 絞っただけでも「自分で絞った」ことになり、AI にジャンルを選ばせなくなっていた。
 * 組む範囲がジャンルだけで決まる以上、ここも揃える。
 */
function 自分で絞っているか() {
  return Object.keys(生成のジャンル絞り()).length > 0;
}
...
let 確かめる候補の言葉 = null;
/*
 * ★辿った言葉の一覧を開いているか。**ふだんは閉じておく**（2026-08-31 本人の指摘）。
 *   > 響きをたくさんやると編集欄がたくさん並んで下の画面を圧迫する
 *   > 目的は画面の圧迫の回避と並びの美しさでした
 * 閉じていても、列で選んだ言葉の行だけは出る。だから普段は 1 行で収まる。
 */
let 言葉の一覧を開く = false;
...
function 響きの欄を描く() {
  ...
  開閉.title = 言('全部並べて見比べたいときだけ開きます。')
    + 言('ふだんは、下の「辿った言葉」の列で選んだものだけが出ます');
   ...
   *
   * ★出す行は「列で選んでいる言葉」のぶんだけ（2026-08-31 本人の指摘）。
   *
   *   > 別ウインドウが目的ではなく、響きをたくさんやると編集欄がたくさん並んで
   *   > 下の画面を圧迫するのでそれを回避したいという意味で、案として提案しました。
   *   > 目的は画面の圧迫の回避と並びの美しさでした。
   *
   * ★並べていた一覧は、すぐ下の「辿った言葉」の**列と同じもの**だった。
   * 同じ一覧が画面に 2 つあり、片方だけが言葉の数だけ伸びていた。
   * **列で選ぶ。板で押す。** 分ければ、言葉が何本あっても普通は 1 行で収まる。
   *
   * ★「辿った言葉 N 本」を開けば、今までどおり全部並ぶ（見比べたいとき用）。
   * ★どちらでもないとき（何も選ばず、閉じている）は、板を出さない。
   */
  const 選ばれ = (sel.言葉 && sel.言葉.size) ? sel.言葉 : null;
  const 選んだ木 = 選ばれ ? 木たち.filter((e) => 選ばれ.has(小文字(e.keyword))) : [];
  const 出す木 = 言葉の一覧を開く ? 木たち : 選んだ木;
  if (!出す木.length) return;
  ...
  for (const e of 出す木) {
    ...
    組む.onclick = async () => {
      ...
      /*
       * ★組む先は、**押したこのボタンが決める**（2026-09-02）。
       * 画面のどのタブを見ていても、この言葉で辿ったものから組む。
       * 前は sel に入れて画面ごしに効かせていたので、
       * タブを切り替えただけで組む先が変わっていた。
       */
      組みの絞り = { 言葉: new Set([小文字(e.keyword)]) };
      /* ★画面にも同じものを出す。何から組んだかが、あとから見て分かるように */
 ...
 *
 * ★まとめてあれば、**まとめた名前**を渡す（2026-08-31 本人の希望）。
 *
 *   > 僕が体感的に使ってほしいのはAIにお願いしてジャンルをまとめたタブです。
 *   > デフォルトはジャンルが散らばりすぎてAIの検索から漏れるものが多いからです。
 *
 * ★測った数字と一致する。そのままは 98 個で、"Rock" は受け皿。
 * Grindcore / GrindCore のような書かれ方の揺れもある。まとめれば 18 組。
 * ★まとめていなければ、今までどおりそのままの名前。
 */
function まとめてあるか() {
  return !!(ジャンルのまとめ && Array.isArray(ジャンルのまとめ.組) && ジャンルのまとめ.組.length);
}
...
function AIに渡すジャンル() {
  ...
  for (const t of 見える曲()) {
    const 素 = (t.genre || "").trim();
    if (!素) continue;
    const g = 束ねる ? (まとめた名(まとめ索引, t.genre) || 素) : 素;
    if (!g) continue;
    数.set(g, (数.get(g) ?? 0) + 1);
  }
...
function 気分の欄を描く() {
  ...
  /*
   * ★「この曲から辿る」も、ここで見直す。
   * キーを入れた直後は AIが使える が切り替わるので、
   * 曲を変えるまで札が出ない ―― という取りこぼしを防ぐ。
   */
  この曲から辿るボタンを直す();
  ...
  const 気分で = async () => {
    ...
    if (!v) return;
    /*
     * ★組む範囲は、押すたびに決め直す（2026-09-02）。
     * ここは「気分から組む」なので、範囲はジャンルの選択に従う。
     * 前の生成で AI が絞った演者が残っていると、次も同じ演者から組んでしまう。
     */
    組みの絞り = null;
    止める(true);
  ...
  const 辿って = async () => {
    ...
    try {
       ...
       */
      /* ★いま辿った組を開いておく。辿った直後は、いま出たものが見たい */
      await 無い名前を貯める(辿った名前 || v);
      /*
       * ★🌱（いま鳴っている曲から辿る）では、画面も再生も動かさない
       *   （2026-09-04 本人の報告）。
       *
       ...
       *   >   次の曲で bold が再生になった
       *
       * ★下の「開いているID = null」が原因。再生リストを閉じると、
       * 流す列が「響きで当たった曲」に入れ替わる（いまの列() は開いている
       * 再生リストを優先し、無ければ絞った結果を返すので）。
       * **聴きながら調べただけなのに、次に流れるものが変わってしまう。**
       *
       * ★🌱 で欲しいのは「手元に無いバンド」の一覧だけ。
       * タブも絞りも再生リストも触らない。押したボタンで振る舞いを決める
       * ―― 0.42.0 で入れた考え方と同じ。
       * ★打ち込んで辿るときは今までどおり（あちらは見に行くための操作なので）。
       */
      if (!いまの曲から辿った) {
        カラムタブ = "resonance";
        /*
       ...
       */
      /*
       * ★🌱 のときは絞っていないので、絞った件数を言わない（言うと嘘になる）。
       * 代わりに、どこへ入ったかを言う ―― 画面が何も動かないので、
       * 何か言わないと「押したのに何も起きない」に見える。
       */
      if (いまの曲から辿った) {
        $('status').textContent = 言('「{言葉}」から辿りました。手元に無いバンドは、上の 📋 に入っています', { 言葉: v });
      } else {
...
function 当てはめる(結果, 要る組数 = 0) {
  ...
  sel = {
    /* ★まとめてあれば、まとめの列に入れる（AI にもまとめた名前を渡しているので） */
    genre: (!演者で && !まとめてあるか() && 結果.ジャンル.length) ? new Set(結果.ジャンル.map(小)) : null,
    まとめ: (!演者で && まとめてあるか() && 結果.ジャンル.length) ? new Set(結果.ジャンル.map(小)) : null,
  ...
  /*
   * ★AI が絞ったぶんは、**内部の絞り込みにも入れる**（2026-09-02）。
   *
   * 組む範囲は画面を見なくなったので、ここで sel に入れるだけでは
   * 次の「一本を組む」に効かない。**AI が挙げた演者から組む**という、
   * この機能の要が抜け落ちる（実測で、演者で絞ると当たりが桁違いに良い）。
   * 画面は「何で絞ったか」を見せるため、内部はそこから組むために持つ。
   */
  組みの絞り = 演者で
    ? { artist: new Set(結果.演者.map(小)) }
    : (結果.ジャンル.length
      ? { [まとめてあるか() ? 'まとめ' : 'genre']: new Set(結果.ジャンル.map(小)) }
      : {});
  列の起点 = { genre: null, artist: null, album: null, 年: null, 月: null, 日: null };
  列の絞り = { genre: "", artist: "", album: "", 年: "", 月: "", 日: "" };
  /* ★まとめてあれば、まとめのタブを見せる（そこで絞っているので） */
  カラムタブ = 演者で ? "tag"
    : (結果.ジャンル.length ? (まとめてあるか() ? "まとめ" : "tag") : "date");
  描き直す();
 ...
 * 3 カラムの中身を切り替えるタブ。
 *
 * ★効くのは、**開いているタブの選択だけ**（2026-08-31）。
 * 隠れている側に残っている数は ● で出すが、**それは効いていない。**
 * 「押せば戻ってくる」ことが分かればよく、いま効いてはいけない。
 */
/**
...
function カラムタブを描く() {
  ...
  for (const [名, 表示] of 札) {
    ...
    if (数 && カラムタブ !== 名) {
      ...
      m.textContent = `● ${数}`;
      b.appendChild(m);
      b.title = 言('このタブに {n} 個残っています（いまは効きません。押すと効きます）', { n: 数 });
    }
    /*
    ...
    b.onclick = () => {
      カラムタブ = 名;
      タブを触った = true;   // ★自分で選んだ。以後、既定で上書きしない
      開いているID = null;
      描き直す();
    };
    box.appendChild(b);
 ...
 */
/*
 * ★いま鳴っている曲から、押すだけで辿る（2026-09-04 本人の希望）。
 *
 *   > 僕の友人で音楽を探すのが苦手な人がいていつも僕に聞いてくるんですが、
 *   > この前chatgptに相談してオススメを聞いたらみんなハズレで困ったと聞いて
 *   > プレイヤーを通じて、「今聴いてるので他にオススメある？」って聞けると、
 *   > その友人も困らないのかな？って思いました。
 *
 * ★足すのは**入口だけ**。辿る仕組みはそのまま使う。
 * 欄に演者名を入れて「辿る」を押すのと、起きることは 1 ミリも変わらない。
 * 変えたのは「何と打てばいいか分からない」を消したところ。それが最初の壁だった。
 *
 * ★実測でも、ここが効く見込みは高い（2026-09-04、6 回ぶん）。
 * 「Helmet みたいな」を起点にすると、Opus は Quicksand / Shellac /
 * Barkmarket / Season to Risk / Shiner を挙げた ―― どれも的確で、
 * **手元に無いものが「探しに行く名前」になる。**
 * ジャンル名を起点にすると外れる。起点が演者名であることが効いている。
 *
 * ★札に演者名を出す（「🌱 Helmet から辿る」）。
 * 押すものが増えるので、**押し間違えても何が起きるか読めるようにしておく。**
 */
...
let 無い名前の開いた組 = null;
/*
 * ★🌱 から辿ったか（2026-09-04 本人の報告）。
 * 🌱 のときは、タブも絞りも再生リストも触らない ―― 聴きながら調べただけで、
 * 次に流れるものが変わってしまう不具合が出た。押したボタンで振る舞いを決める。
 */
let いまの曲から辿った = false;
...
function 起点のジャンル(起点) {
  /*
   * ★末尾の (01) を落としてから当てる（2026-09-04）。
   * 同じ言葉で 2 度 辿ると helmet(01) のように名前が変わるので、
   * そのままだと手元の演者に当たらず、ジャンルが引けなくなる。
   */
  const k = 小文字(String(起点 || '').trim().replace(/\(\d+\)$/, '').trim());
  if (!k) return '';
  const 数 = new Map();
...
function この曲から辿るボタンを直す(t = いま鳴っている曲) {
  const b = $('nowtrace');
  if (!b) return;
  const 演者 = String((t && t.artist) || '').trim();
  /* ★AI が使えないとき・演者名が無いときは、出さない（押せないものを見せない） */
  if (!AIが使える || !演者) { b.style.display = 'none'; return; }
  b.style.display = '';
  b.textContent = 言('🌱 {名前} から辿る', { 名前: 演者 });
  b.title = 言('この演者から辿れる音楽を探します。手元にあったものは響きタブに、')
    + 言('無かったものは「確かめる候補」に出ます（一本は組みません）');
  b.onclick = () => {
    /*
     * ★欄に入れてから、いつもの「辿る」を押す。
     * 別の道を作らない ―― 2 か所で辿ると、片方だけ直す事故になる。
     */
    const 欄 = $('restree');
    打ちかけの辿る言葉 = 演者;
    if (欄) 欄.value = 演者;
    const 押す = $('restreego');
    /*
     * ★これは「聴きながら調べる」操作。画面も再生も動かさない印を立てる
     *   （2026-09-04 本人の報告。次に流れる曲が入れ替わっていた）。
     */
    いまの曲から辿った = true;
    if (押す) 押す.click();
    else { いまの曲から辿った = false; $('status').textContent = 言('⚠ 先に APIキーを入れてください'); }
  };
}
...
function 無い名前の札を直す() {
  ...
  b.style.display = '';
  b.textContent = 言('📋 手元に無いバンド（{n}）', { n: 出す.length });
  b.title = 言('辿って見つかったバンドのうち、手元に無かったものです。')
    + 言('響きを外しても残ります');
...
function 無い名前の欄を描く() {
   ...
   *
   * ★ここで先に組んでおく。書き写すボタンからも同じ並びを使うので。
   */
  const 組ごと = new Map();
  for (const x of 出す) {
  ...
  題.textContent = 言('手元に無いバンド（{n}）', { n: 出す.length });
  ...
  写.className = 'restag';
  写.textContent = 言('⧉ コピー');
  写.title = 言('一言 解説つきで、ジャンルごとに書き写します');
  写.onclick = async () => {
    /*
     * ★一言 解説も入れる（2026-09-04 本人の希望）。
     *   > コピペしてみたけどバンド名だけは寂しかった。
     *   > リストの横の一言解説はかなりありがたく重要なのでこれもコピペしたい。
     *
     * ★最初はバンド名だけにしていた（検索窓へそのまま貼れるように）。
     * だが実際に使うと、貼った先で**どれが何だか分からない**。
     * 解説こそが「探しに行くかどうか」を決める材料だった。
     * ★ジャンルの見出しも入れる。控えとして読めるほうが値打ちがある。
     */
    const 文 = 並び.map(([見出し, 中身]) => [
      `【${見出し}】`,
      ...中身.map((x) => (x.description ? `${x.name} ― ${x.description}` : x.name)),
    ].join('\n')).join('\n\n');
  ...
  const 頭 = document.createElement('div');
  頭.className = 'foot';
  頭.textContent = 言('辿って挙がったバンドのうち、手元で見つからなかったものです。')
  ...
  足.onclick = async () => {
    if (!await 確かめる(言('手元に無いバンドを、全部 一覧から外しますか？')
    ...
    無い名前を開く = false;
    無い名前の欄を描く();
    $('status').textContent = 言('手元に無いバンドを、全部 外しました（戻せます）');
  };
  面.appendChild(足);
 ...
 *
 * ★いま辿った組を開いておく（2026-09-04 本人の希望）。
 *   > バンドから辿るを実行した場合、その時に生成したものが開いてくれると嬉しい
 * いちばん多い組が開くのが既定だが、辿った直後は**いま出たものが見たい。**
 *
 * @param 辿った言葉 いま辿った言葉。渡せば、その組を開いた状態にする
 */
async function 無い名前を貯める(辿った言葉 = '') {
  ...
  /*
   * ★いま辿った組を開く。見出しの作り方は 無い名前の欄を描く() と同じにする
   * （食い違うと、開けと言った組が無いことになって、何も開かない）。
   */
  if (辿った言葉) 無い名前の開いた組 = 起点のジャンル(辿った言葉) || 辿った言葉 || 言('その他');
  無い名前の欄を描く();
}
