 ...
 *
 * ★完全一致で見る。部分一致にすると、その名前を**含む別のバンド**まで
 * 巻き込む（短い一語の名前だと実際に起きた。10 曲ほど巻き込んだ）。
 *
 * ★曲は消さない。一覧にも残る。押せば鳴る。
...
function つまみの行() {
  ...
  const 覚える = async () => {
    ...
    /*
     * ★強度は響きの並びにも効く（2026-08-31）。動かしたら並べ直す。
     * 動かしたのに響きタブが前のままだと、効いていないように見える。
     */
    if (響きの木) { 響きを合わせ直す(); 描き直す(); }
  };
  ...
  /*
   * ★拡大解釈は、強度とは別の軸（2026-08-30 本人の希望）。
   ...
   *
   * 分けたことで「王道のものを、遠くまで広げて選ぶ」が言えるようになった。
   */
  const 広げる = document.createElement("button");
  ...
  広げる札();
  広げる.onclick = async () => {
    const 次 = !(AIのつまみ.目盛.拡大解釈 === true);
    AIのつまみ.目盛 = { ...AIのつまみ.目盛, 拡大解釈: 次 };
    広げる札();
    AIのつまみ.目盛 = await window.mp3.AIのつまみを変える(AIのつまみ.目盛);
    広げる札();
  };

  /*
   * ★名前をやめて、左端を出す（2026-08-30 本人の希望、画面の写真つき）。
   *
   *   > スライダーの名前の表示をなんとかしたいなと思いました。
   *   > 狭い↔広い、10曲↔80曲、王道↔外す、みたいな表現だと
   *   > 使い方わからないですかね？ 表示をシンプルにしたいです。
   *
   * ★写真では名前が 2 行に折り返していた（「対象／の幅」「文脈の強／度」）。
   * 幅が足りないのに名前を置いていたのが原因。
   *
   * ★右側は**もともと現在値**（広い／80 曲／外す）。
   * だから足りないのは左端だけで、挙げてもらった例がちょうどその形になる。
   *   狭い ―●― 広い ／ 10 曲 ―●― 80 曲 ／ 王道 ―●― 外す
   *
  ...
  /*
   * ★両端。「狭い ―●― やや広い」のように片方を今の値にすると、
   * **範囲に見えて紛らわしい。** 両端なら、つまみの位置がそのまま今を表す。
   * ★いまの値は、説きが数で出している（候補 400 曲 … → 80 曲の一本）。
   */
  行.append(端(幅の左), 幅棒, 端(幅の右), 端(量の左), 量棒, 端(量の右),
    端(強の左), 強棒, 端(強の右), 広げる, 説き);

  /*
   * ★今の値の札は、行には出さないが**残しておく**。
   * 検査と、あとから読みたくなったときのため。表に 3 つ散らすのはやめた。
   */
  for (const e of [幅札, 量札, 強札]) { e.style.display = "none"; 行.appendChild(e); }
...
function AIに渡す候補(何曲, 一組 = 1) {
  ...
  /*
   * ★ここから先は、**シャッフルとは別のくじ**です（2026-08-31 本人の希望）。
   *
   *   > シャッフルと、AI DJ や Resonance からのプレイリスト作成のロジックは分けたい
   *
   * シャッフルは「再生回数が少ないほど重い」＝忘れている曲を掘り起こす道具。
   * 一本を組むのは逆で、**その演者の核となる曲**が要る。目的が違う。
   * だから再生回数は**見ません**。
   *
   * ★ついでに分かったこと: この蔵書では 86,044 曲のうち
   * 再生回数が付いているのは **8 曲だけ**。重み 1/(n+1) は、ほぼ全曲で 1 だった。
   * つまり回数の重みは、もともとここでは効いていなかった。
   */

  const 盤の名 = (p) => { const t = 表.get(p); return 小文字((t.artist || '') + ' / ' + (t.album || '')); };

  /* 盤ごとの曲数。位置（何曲目か）を測るのに要る */
  const 盤の長さ = new Map();
  for (const t of 母集団) {
    const k = 盤の名(t.path);
    const n = typeof t.track === 'number' ? t.track : 0;
    盤の長さ.set(k, Math.max(盤の長さ.get(k) ?? 0, n));
  }

  /*
   * ★名曲が置かれやすい場所（2026-08-31 本人の見立て）。
   *
   ...
   *
   * ★測ったら、そのとおりだった（この蔵書、番号の通った盤 6,743 枚）。
   * 題が intro / outro / interlude … の曲が、どこに多いか:
   *
   *   1 曲目 6.78%（その他の 10.5 倍）   2 曲目 0.31%（盤の中で一番きれい）
   *   真ん中 0.89%                       最後   2.04%（3.1 倍）
   *   その他 0.65%
   *
   * ★題で分かるつなぎは、上の つなぎか() ですでに外してある。
   * ここで効かせるのは、**題では分からないぶん**の博打度。
   * 1 曲目を 2 曲目・真ん中より下げているのは、そのため。
   */
  const 場所の点 = (番, 長さ) => {
    if (!番 || !長さ || 長さ < 4) return 1;          // 短すぎる盤に位置は無い
    if (番 === 2) return 3;                          // 現代のオープニング
    if (番 >= Math.floor(長さ / 2) && 番 <= Math.ceil(長さ / 2) + 1) return 3;   // A 面の終わり／B 面の頭
    if (番 === 1) return 2;                          // 名曲だが、無題のイントロが残る
    if (番 === 長さ) return 1;                       // 博打
    return 1;
  };
  ...
  /*
   * ★同じ題が、その演者の 2 枚以上に入っている曲。
   * ベスト盤やコンピに再録されたということなので、**蔵書そのものが投票している。**
   * 実測: 2 枚以上持っている演者 1,730 組のうち 680 組（39%）にある。
   */
  const 再録 = new Set();
  for (const [, 道] of 束) {
    const 題ごと = new Map();
    for (const x of 道) {
      const t = 表.get(x);
      const q = 小文字((t.title || '').replace(/\s*\(.*?\)\s*$/, ''));
      if (!q) continue;
      if (!題ごと.has(q)) 題ごと.set(q, []);
      題ごと.get(q).push(x);
    }
    for (const [, xs] of 題ごと) {
      if (new Set(xs.map(盤の名)).size >= 2) xs.forEach((x) => 再録.add(x));
    }
  }

  /* ★選ばれた曲だけを集めた盤。入っている時点で、誰かが選んでいる（実測 5.0%） */
  const 選ばれた盤 = /(best|greatest|hits|singles|collection|anthology|essential|discograph|compilation|ベスト)/i;

  /** その演者の核に近いほど大きい（0〜8）。★再生回数は見ない */
  const 核の点 = (x) => {
    const t = 表.get(x);
    if (!t) return 0;
    let 点 = 場所の点(typeof t.track === 'number' ? t.track : 0, 盤の長さ.get(盤の名(x)) ?? 0);
    if (再録.has(x)) 点 += 3;
    if (選ばれた盤.test(String(t.album || ''))) 点 += 2;
    return 点;
  };

  /*
   * 次を選ぶ() は「表の値が小さいほど重い」（1/(n+1)）ので、
   * 欲しい重み w に対して n = 1/w - 1 を入れてやる。
   * 響きで当たった曲は、そのぶん重ねる。
   */
  /*
   * ★重みは 1 を超えられない（2026-08-31、測って気づいた）。
   * 次を選ぶ() の重みは 1/(n+1) なので、**どう頑張っても最大 1。**
   * 素朴に n = 1/w - 1 と置くと、w が 1 を超えた瞬間に n が負になり、
   * Math.max(0, …) で 0 に潰れて**全曲が同じ重みになる。**
   * 実際そうなっていて、位置の重みがまったく効いていなかった
   * （前後で 2 曲目 10%→11%。動いていない）。
   *
   * いちばん重い曲を 1 に合わせてから割り当てる。
   */
  const 重みを表に = (重み) => {
    const w = new Map(母集団.map((t) => [t.path,
      Math.max(0.0001, 重み(t.path) * ((響き表 && 響き表[t.path]) || 1))]));
    const 最大 = Math.max(...w.values());
    return Object.fromEntries([...w].map(([x, v]) => [x, 最大 / v - 1]));
  };
  const 核の表 = 重みを表に((x) => 核の点(x) + 1);
  /* ★スパイスの一杯。核ばかり並べると、聴いたことのある曲だけになる */
  const 平らな表 = 重みを表に(() => 1);

  const 演者 = [...束.keys()];
  /*
   * ★1 組から何曲か渡すので、組そのものを先に絞る（2026-08-31）。
   * 200 組×1 曲だと、AI は**どの曲を出すか選べない。**
   * 67 組×3 曲なら、その演者の代表曲を AI が選べる。
   * 組の数は、並べる曲数の 2 倍以上は残るようにしてある（ai.js の 一組の曲数）。
   */
  const 組の上限 = Math.max(1, Math.ceil(上限 / Math.max(1, 一組)));
  const 選ぶ組 = (響いた組.size
  ...
  for (let 巡 = 1; 巡 <= 上限 && 出.length < 上限; 巡 += 1) {
    ...
    for (const k of 並べ直す()) {
      ...
      /*
       * ★最後の 1 杯だけ、重みを外す（スパイス）。
       * 核ばかり並べると、知っている曲だけの一本になる。
       * 本人の言葉:「そのバンドのコアとなる曲と、時々スパイスとなる曲の組み合わせ」
       */
      const 表を使う = (一組 >= 2 && 巡 === 一組) ? 平らな表 : 核の表;
      const p = 次を選ぶ(使える.length ? 使える : 残り, 表を使う, new Set());
      if (!p) continue;
      引いた.add(p);
  ...
  /*
   * ★演者ごとにまとめてから番号を振る（2026-08-31 本人の報告）。
   *
   *   > 同じバンドや同じバンドの同じアルバムからの選曲が多くなりました
   *
   * 引くのは巡ごと（1 周目に全組から 1 曲、2 周目にもう 1 曲…）なので、
   * そのまま並べると**同じ演者が 67 行おきに散らばる。**
   * AI からは「同じ組の 3 曲」ではなく「別々の良い曲 3 つ」に見えてしまう。
   *
   * まとめて渡せば、1 組がひとかたまりで見える。
   * ★組の順は引いた順のまま（あいうえお順にすると、並びに癖が出る）。
   */
  const 組ごと = new Map();
  for (const x of 出) {
    const k = 小文字((x.artist || '').trim()) || ('\u0000' + x.path);
    if (!組ごと.has(k)) 組ごと.set(k, []);
    組ごと.get(k).push(x);
  }
  const 並べた = [];
  for (const [, xs] of 組ごと) for (const x of xs) 並べた.push(x);
  並べた.forEach((x, i) => { x.番号 = i + 1; });
  return 並べた;
}
...
async function AIに一本組ませる(気分, 言った) {
  ...
  const 候補 = AIに渡す候補(大きさ.候補の数, 大きさ.一組の曲数 || 1);
  /*
   * ★演者で絞っているときは、**組の数より多くは頼まない**（2026-08-31）。
   * 1 組 1 曲が決まりなので、20 組しかいないのに 30 曲頼めば必ず重なる。
   * 短い一本のほうがいい ―― 減らしたことは画面に出る。
   */
  const 組の数 = (sel.artist && sel.artist.size) || 0;
  if (候補.length < 2) {
   ...
   */
  /*
   * ★減った理由を、正しく言い分ける（2026-08-31）。
   * 演者で絞っているときは「候補が足りない」ではなく「組が足りない」。
   * 前は候補のせいにしていて、**間違った理由を出していた。**
   */
  const 減り = (() => {
    if (!r.結果.頼んだ曲数 || r.結果.頼んだ曲数 >= 大きさ.作る曲数) return "";
    if (組の数 && r.結果.頼んだ曲数 === 組の数) {
      return 言('（この気分で手元にあったのが {n} 組なので、{n} 曲にしました）', { n: 組の数 });
    }
    return 言('（候補が {候補} 曲しかないので {頼んだ} 曲にしました。対象の幅を広げると増やせます）',
  ...
  const 散らし = 重なり
    ...
    : 言('（すべて別の演者です）');
  /*
   * ★重なりを落としたら、そう言う（2026-08-31）。
   * 黙って短くすると「頼んだ曲数より少ない」だけが見えて、理由が分からない。
   */
  const 重なり落ち = r.結果.重なり落ち
    ? 言('（同じ演者・同じ盤が {n} 曲重なっていたので落としました）', { n: r.結果.重なり落ち })
    : '';
  const 打ち止め = 頭打ち
    ? 言('　※ この範囲は {n} 曲で全部なので、対象の幅を広げても増えません', { n: 候補.length })
    : '';
  $('status').textContent = 言('AI が {n} 曲の一本を組みました: 「{名}」{添え}　▶ で頭から流れます',
    { n: 道.length, 名, 添え: 散らし + 落ち + 重なり落ち + 減り + 打ち止め });
  return true;
}
...
/*
 * ★深さの向きは、つまみ（王道↔外す）で決まる（2026-08-31 本人の希望）。
 *
 *   > 響きを知覚を濃くするためにつまみに繋げてください。
 *
 * ★覚え直さず、つまみから**直に読む。**
 * 控えを持つと、つまみを動かしたのに古い値のまま、という食い違いが起きる。
 * 取れていなければ「ふつう」（3）。
 */
const 響きの目盛 = () => {
  const v = AIのつまみ && AIのつまみ.目盛 && AIのつまみ.目盛.強度;
 ...
 */
/* ★直前に辿ったとき、実際に保存された名前（helmet(01) など） */
let 辿った名前 = '';
...
/*
 * ★突き合わせ用の演者一覧（2026-08-31）。
 *
 * ★これは**外に送りません。** AI が知識から挙げた名前を、
 * 手元にあるかどうか照らすためだけに使います。
 * 送るのは今までどおり ―― ジャンル名・年・気分の文・候補 200 曲だけ。
 */
function AIに渡す演者() {
  const 名 = new Set();
  for (const t of 見える曲()) { const a = (t.artist || '').trim(); if (a) 名.add(a); }
  return [...名];
}
...
function 気分の欄を描く() {
  ...
  const 止める = (と) => {
    ...
    for (const id of ["aiwide", "aimany", "aistrict", "aiwide2"]) { const e = $(id); if (e) e.disabled = と; }
  };
  ...
  const 気分で = async () => {
    ...
    try {
      ...
      if (自分で絞っているか()) {
        ...
        当てはめる(r.結果, (await window.mp3.AIの大きさ()).作る曲数);
      }
  ...
  const 辿って = async () => {
    ...
    try {
       ...
       * 0.37.0 から、同じ言葉なら helmet(01) のように名前が変わる。
       * 打った言葉のまま絞ると 0 件になり、辿ったのに何も出ない。
       */
      const 絞る言葉 = 辿った名前 || v;
      sel = { ...sel, 言葉: new Set([小文字(絞る言葉)]), 響演者: null, 響盤: null };
      /*
 ...
 * 気に入らなければ、そのまま押して外せる。
 */
function 当てはめる(結果, 要る組数 = 0) {
  const 小 = (v) => String(v).toLocaleLowerCase("ja");
  /*
   * ★演者で絞れるなら、そちらを使う（2026-08-31 本人との話から）。
   *
   *   > 「Helmet みたいなバキバキのオルタナロック」
   *
   * ★いちばん情報量が多いのは「Helmet みたいな」なのに、
   * ジャンル名 2 つに翻訳した時点で**捨てていた。**
   * しかもこの蔵書では、広い意味のロックが 9 割方。「ロック」で絞っても絞れない。
   * 実測では「Rock」の札が付いているのは 4% で、中身は
   * Godspeed / Puffy / Bauhaus / Napalm Death のライブ盤まで混ざっていた。
   *
   * ★はじめ「並べる曲数ぶんそろったときだけ」にしていたが、**厳しすぎた**
   * （2026-08-31 実地。30 組そろわず、毎回ジャンルに落ちていた）。
   *
   * 数組でも当たれば、そちらのほうがずっと近い。
   * 足りないぶんは**一本を短くする** ―― 同じ演者を並べるより、短いほうがいい。
   * 短くしたことは画面に出る。
   */
  const 演者で = (結果.演者 || []).length >= 6;
  sel = {
    genre: (!演者で && 結果.ジャンル.length) ? new Set(結果.ジャンル.map(小)) : null,
    artist: 演者で ? new Set(結果.演者.map(小)) : null,
    album: null,
  ...
  カラムタブ = (演者で || 結果.ジャンル.length) ? "tag" : "date";
  描き直す();

  const 何曲 = 絞る(3).length;
  /* ★何で絞ったかを、はっきり出す。ジャンルと演者では意味が違う */
  const 選んだ = 演者で
    ? 言("演者 {n} 組: {名前} ほか", {
      n: 結果.演者.length, 名前: 結果.演者.slice(0, 4).join(" / "),
    })
    : ([...結果.ジャンル, ...結果.年].join(" / ") || 言("（絞り込みなし）"));
  /*
   * ★挙げた数と、手元に無かった名前を出す（2026-08-31 本人の報告）。
   *
   *   > まだ拡大解釈な感じです。
   *   > こちらの環境や設定が悪いかもしれないので、そこも考慮して
   *
   * ★実測すると、そのとおりだった。Helmet と同じ棚の 26 組のうち、
   * 手元にあるのは 5 組（19%）。一本に入った 18 組は 100% 持っていた。
   * **AI は正しい名前を挙げ、突き合わせで落ちていた。**
   *
   * つまみをどう振っても、持っていないものは出せない。ならばそう言う。
   * 黙って持っている側だけ出すから、「拡大解釈」に見える。
   */
  const 当たり具合 = (演者で && 結果.演者を挙げた数)
    ? 言("　※ AI は {挙げた} 組挙げましたが、手元にあったのは {あった} 組でした", {
      挙げた: 結果.演者を挙げた数, あった: 結果.演者.length,
    }) + ((結果.演者の外れ || []).length
      ? 言("（{名前} などは持っていません）", {
        名前: (結果.演者の外れ || []).slice(0, 4).join(" / "),
      }) : "")
    : "";
  ...
  $("status").textContent = 言('AI が選んだ範囲: {選んだ} ― {n} 曲{無し}　この中から一本を組んでいます…',
    { 選んだ, n: 何曲.toLocaleString("ja-JP"), 無し: 無し + 当たり具合 });
}
...
function 描き直す() {
  ...
  /*
   * ★下の 3 つも、ここから書き換える（2026-08-30 本人の画面写真）。
   * どれも「中身が変わったとき」にだけ直す作りだったので、
   * 英語にしても日本語のまま残っていた。
   */
  タグ無しのボタンを直す();
  再スキャンのボタンを直す();
 ...
 */
/** ★タグ無しの札。言葉が変われば書き換わるよう、1 か所にまとめてある */
function タグ無しのボタンを直す() {
  const b = $('untagged');
  if (!b) return;
  b.textContent = タグ無しを隠す ? 言('🏷 タグ無しを隠す') : 言('🏷 タグ無しも出す');
}
