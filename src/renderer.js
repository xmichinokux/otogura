...
let 再生回数 = {};
/**
 * いま流している並び。曲を押した時点の一覧を覚えておく。
 * ★これを覚えないと、再生中に絞り込みを変えたときに「次の曲」が変わってしまう。
 */
let 流している列 = [];
/** シャッフルで、この巡に流した曲 */
let 巡 = new Set();
...
function 一覧を描く() {
  ...
  const 対象 = リスト
    ? リスト.tracks.map((p) => tracks.find((t) => t.path === p)).filter(Boolean)
    : 絞る(3).sort(曲を並べる);
...
function タグ編集を描く(box) {
  ...
  書く.onclick = async () => {
    ...
    if (!confirm(
      `${数.toLocaleString('ja-JP')} 曲の MP3 ファイルを書き換えます。\n\n${中身}\n\n`
...
/**
 * @param 列を保つ true なら「いま流している並び」を作り直さない
 *   （自動送りのとき。押した時点の並びのまま進めたいので）
 */
function 再生する(t, { 列を保つ = false } = {}) {
  // ★押した時点の一覧を覚える。覚えないと、再生中に絞り込みを変えたら次の曲が変わる
  if (!列を保つ) {
    const リスト = 開いているリスト();
    流している列 = リスト
      ? リスト.tracks.map((p) => tracks.find((x) => x.path === p)).filter(Boolean)
      : 絞る(3).sort(曲を並べる);
    巡 = new Set();                              // 並びが変わったらシャッフルの巡も仕切り直す
  }
  巡.add(t.path);
  数えた = false;
...
function 送る(方向) {
  const t = 次の曲(方向);
  if (t) 再生する(t, { 列を保つ: true });
  else {
    audio.pause();
    $('sub').textContent = '最後まで再生しました';
  }
}
