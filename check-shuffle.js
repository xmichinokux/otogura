 ...
 * ★頭は 値たち から（2026-09-05）。タブに合う がそれを使うので、
 * タブに合う から切ると ReferenceError になる。
 */
const 始め5 = 画面.indexOf('function 値たち(');
...

{
   ...
   */
  /*
   * ★2026-09-02、組む範囲が画面を見なくなった（本人の指定）。
   *   > AI DJと響きは画面に表示されてるリストを参照しない。内部でリストを参照する
   * 欄も、実際に効いているものを書かないと食い違う。
   */
  確認4(
    '★組む範囲の欄は、実際に効いている絞り込みを並べる',
    画面.includes('const 効く絞り = 組みの絞り || 生成のジャンル絞り();')
      && 画面.includes('const 絞り = Object.entries(効く絞り)'),
    'sel を並べると、曲数と食い違います（組む範囲は sel を見ていません）',
  );
  確認4(
  ...
  /*
   * ★組む範囲は、ジャンルの選択だけで決まる（2026-09-02 本人の指定）。
   *
   *   > ジャンルを指定していなければ全てを参照するけど、ジャンルの指定を
   *   > していたらそのジャンル内のみ参照する。しかし、画面に表示されているか
   *   > されていないかは関係なく、あくまで内部でリストを参照する
   *
   * ★0.40.0 で隠れた絞り込みは潰したが、**タブを開いている状態そのものが
   * 範囲を決める**構造は残っていた。それが生成の誤操作の最後の源だった。
   */
  確認4(
    '★組む範囲は 絞る() を通らない（画面を見ない）',
    /function 組む範囲\(\) \{[\s\S]{0,240}?見える曲\(\)\.filter\(\(t\) => 組みに合う/.test(画面)
      && !/function 組む範囲\(\) \{[\s\S]{0,240}?絞る\(3\)/.test(画面),
    '絞る(3) を通すと、開いているタブで組む先が変わります',
  );
  確認4(
    '★ジャンルはまとめ優先、日付は見ない',
    画面.includes("const 鍵 = まとめてあるか() ? 'まとめ' : 'genre';")
      && !/function 生成のジャンル絞り\(\)[\s\S]{0,300}?sel\.年/.test(画面),
    '散らかった 98 個から組むと、AI の検索から漏れます',
  );
  確認4(
    '★AI が絞った演者は、内部の絞り込みにも入れる',
    /組みの絞り = 演者で[\s\S]{0,120}?artist: new Set\(結果\.演者\.map\(小\)\)/.test(画面),
    '入れないと「AI が挙げた演者から組む」が効かなくなります',
  );
  確認4(
    '★響きの「一本を組む」は、押したボタンが範囲を決める',
    画面.includes("組みの絞り = { 言葉: new Set([小文字(e.keyword)]) };"),
    '画面ごしに効かせると、タブを切り替えただけで組む先が変わります',
  );
  確認4(
    '★気分から組むときは、前の絞り込みを持ち越さない',
    /const 気分で = async \(\) => \{[\s\S]{0,400}?組みの絞り = null;/.test(画面),
    '前の生成で AI が絞った演者が残ると、次も同じ演者から組んでしまいます',
  );
  確認4(
    '★再生とシャッフルは、今までどおり見えている一覧から（本人の指定は A）',
    画面.includes(': 絞る(3).sort(曲を並べる);'),
    '「見えているもの＝鳴るもの」は変えない、というのが指定でした',
  );
}
