      ...
      setTimeout(async () => {
        ...
        try {
          ...
          const r = await win.webContents.executeJavaScript(`(async () => {
            ...
            /*
             * ★「▶ が押せるか」を、本物の画面で見る（2026-08-29 実地）。
             *
             * ▶ を押すだけで流し始める作りにしたのに、ボタンが disabled のままで
             * **押せなかった**。本人に届いてから分かった:
             *   > シャッフルを選択しても再生ボタンがグレーのままなので再生できませんでした
             *
             * ★check-shuffle は 流し始める() を直に呼んでいたので、素通りした。
             * 「関数が正しい」ことと「押せる」ことは別。ここは押せるかを見る。
             *
             * 曲を仕込んで 一覧を描く() を呼び、ボタンの状態を読む
             * （本人のライブラリが空でも確かめられるように）。
             */
            const 再生ボタン検査 = (() => {
              try {
                const 元 = tracks;
                const 作る = (i, 属性) => Object.assign({
                  path: 'C:/検査/' + i + '.mp3', title: '曲' + i, artist: 'あ', album: 'AL',
                  genre: 'G', track: i, duration: 1, タグあり: true,
                }, 属性 || {});
                const 読む = () => document.getElementById('play').disabled;
                ...
                tracks = [作る(1, { 鳴らせる: false })]; 一覧を描く();
                const 鳴らないだけ = 読む();

                tracks = 元; 一覧を描く();
                return { 曲なしで押せない: 曲なし === true, 曲があれば押せる: 曲あり === false, 鳴らない曲だけなら押せない: 鳴らないだけ === true };
              } catch (e) { return { 失敗: String(e && e.message || e) }; }
            })();
...
子.on('close', () => {
  ...
  /*
   * ★▶ が押せるか（2026-08-29 実地）。
   * 押して流し始める作りにしたのに disabled のままだった。
   * 「関数が正しい」ことと「押せる」ことは別なので、押せるかを見る。
   */
  const 再 = r.再生ボタン検査;
  if (!再 || 再.失敗) {
    確認('▶ の押せる／押せないを確かめられる', false, 再 ? 再.失敗 : '結果が返っていません');
  } else {
  ...
  /*
   * ★キーのボタンに、何のボタンか書いてあるか（2026-08-29 本人の指摘）。
   *   > 今となっては何のボタンかわからないので
   *   > APIキーを登録するボタンだとちゃんと書いたほうがいい
   * もとは「気分で選ぶ」だった。押しても選曲はせず、キーを入れるだけだった。
   */
  確認(
    '★キーのボタンに、何のボタンか書いてある',
    !!(r.キーのボタン && r.キーのボタン.見えている && /APIキー/.test(r.キーのボタン.文字)),
    r.キーのボタン ? `いまの文字:「${r.キーのボタン.文字}」` : 'ボタンがありません',
  );
